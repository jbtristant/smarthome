...
void HomeWorker::init()
{
    ...
    connect(arduinoMegaOne, &QSerialPort::readyRead, this, &HomeWorker::dataReceived);

    if (!arduinoMegaOne->open(QIODevice::ReadWrite)) {
        qDebug() << "Can not open arduino mega one serial port";
        return;
    }
...
void HomeWorker::dataReceived()
{
    bufferMegaOne.append(arduinoMegaOne->readAll());
    processDataReceived();
}

void HomeWorker::processDataReceived()
{
    ...
    foreach (QString line, splitEndLine) {
        ...
        if (line.contains("&relay id") && line.contains("$")) {
            ...
            if (relay.size() == 4) {
                qDebug() << line;
                emit relayChanged(relay.at(2).toInt(), relay.at(3) == "on"? true: false);
            }
        }
